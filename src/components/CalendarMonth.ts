...
export class CalendarMonth extends HTMLElement {
  ...
  isToday(day : number, month : number) { return day == this.today.getDate() && month == this.today.getMonth() + 1; }
  isTodayProp(day : number) { return this.isToday(day, this.month) ? 'today' : ''; }
